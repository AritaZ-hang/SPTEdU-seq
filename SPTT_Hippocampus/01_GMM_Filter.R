...
## load dge
dge = as.data.frame(fread(file = "./Hippocampus_top5W_dge.txt.gz"))
dge = column_to_rownames(dge, "GENE")

# the barcode_mapping.txt.gz is generated by Barcode_Degen workflow
...
meta.filter = meta %>% filter(dist_gmm != "3") %>% filter(UMIs >= 300)
